...
public class LancamentoVenda extends javax.swing.JInternalFrame {
	...
	private void btRemoverItemActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btRemoverItemActionPerformed
		if (tbGradeItens.getSelectedRowCount() > 0) {
			...
			if (JOptionPane.showConfirmDialog(this, "Deseja realmente excluir o item " + iv + "?", "Confirmação",
	...
	private void tbGradeMouseClicked(java.awt.event.MouseEvent evt) {// GEN-FIRST:event_tbGradeMouseClicked
		if (evt.getClickCount() == 2) {
			...
			if (venda.getSituacao() == Situacao.FINALIZADA) {
				JOptionPane.showMessageDialog(this, "Venda finalizada não pode ser alterada.", "Alerta",
						JOptionPane.WARNING_MESSAGE);
				return;
			}
			if (venda.getSituacao() == Situacao.CANCELADA) {
				JOptionPane.showMessageDialog(this, "Venda cancelada não pode ser alterada.", "Alerta",
						JOptionPane.WARNING_MESSAGE);
				return;
			}
	...
	public void setProduto(Produto produto) throws Exception {
		...
		if (codigo == v.getCliente().getCodigo()) {
			if (JOptionPane
					.showConfirmDialog(
							this, "Este cliente já comprou o " + produto
									+ " anteriormente. Será aplicado um desconto de " + produto.getDesconto(),
			...
			JOptionPane.showMessageDialog(null, "ID do cliente não é igual ao Id contido na tabela de venda!");
		}

		if (ClasseProduto.CRONICA.equals(classe)) {

			if (JOptionPane.showConfirmDialog(this, "O medicamento " + produto
					+ " é para doenças crônicas, será aplicado um desconto de " + produto.getDesconto(), "Confirmação",
					JOptionPane.YES_NO_OPTION) == 0) {
			...
			if (JOptionPane.showConfirmDialog(this,
					"O medicamento " + produto + " contém alergénico, deseja adicionar mesmo assim ?", "Confirmação",
					JOptionPane.YES_NO_OPTION) == 0) {
	...
	private boolean validarFormularioItens() {
		...
		if (ftfClasse.getValue() == null) {
			JOptionPane.showMessageDialog(this, "Classe de Doença inválida.", "Alerta", JOptionPane.WARNING_MESSAGE);
			ftfProduto.requestFocus();
			return false;
		}
		return true;
