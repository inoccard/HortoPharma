...
import com.farma.gui.table.ProdutoTableModel;
...
public class CadastroProduto extends javax.swing.JInternalFrame {
	...
	private void initComponents() {
		...
		lbClasse.setText("Classe Doença");
		gridBagConstraints = new GridBagConstraints();
		gridBagConstraints.gridx = 0;
		...
		gridBagConstraints.anchor = GridBagConstraints.LINE_END;
		gridBagConstraints.insets = new Insets(2, 2, 2, 2);
		pnForm.add(lbDesconto, gridBagConstraints);
		...
		ftfDesconto.setColumns(10);
		ftfDesconto.setValue(new Double(0));
		gridBagConstraints = new java.awt.GridBagConstraints();
		gridBagConstraints.gridx = 1;
		gridBagConstraints.gridy = 4;
		gridBagConstraints.anchor = java.awt.GridBagConstraints.LINE_START;
		gridBagConstraints.insets = new java.awt.Insets(2, 2, 2, 2);
		pnForm.add(ftfDesconto, gridBagConstraints);
	...
	private void btSalvarActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btSalvarActionPerformed
		if (validarFormulario()) {
			...
			if (cmbClasse.getSelectedItem().equals("CRONICA")) {
				produto.setClasse(1);
				produto.setDesconto((Double) ftfDesconto.getValue());
			} else if (cmbClasse.getSelectedItem().equals("ALERGIA")) {
				produto.setClasse(2);
				produto.setDesconto(new Double(0.00));
			} else {
	...
	private void tbGradeMouseClicked(java.awt.event.MouseEvent evt) {// GEN-FIRST:event_tbGradeMouseClicked
		if (evt.getClickCount() == 2) {
			...
			if (produto.getClasse().equals(ClasseProduto.CRONICA)) {
				cmbClasse.setSelectedItem(ClasseProduto.CRONICA.toString());
			} else if (produto.getClasse().equals(ClasseProduto.ALERGIA)) {
				cmbClasse.setSelectedItem(ClasseProduto.ALERGIA.toString());
			}
			ftfDesconto.setValue(produto.getDesconto());
			habilitarFormulario(true);
		}
	...
	private void limpaFormulario() {
		produto = null;
		ftfNome.setValue("");
		ftfPrecoCompra.setValue(new Double(0.00));
		ftfPrecoVenda.setValue(new Double(0.00));
		cmbClasse.setSelectedItem("SELECIONE");
		ftfDesconto.setValue(new Double(0.00));
	}
...
/*		if (((Double) ftfPrecoCompra.getValue()) <= 0) {
		...
		// Double value = (Double) ftfPrecoVenda.getValue();
...
/*		if ((Double) ftfDesconto.getValue() < 0) {
			JOptionPane.showMessageDialog(this, "Valor de desconto não pode ser menor que zero ou nulo.", "Alerta",
					JOptionPane.WARNING_MESSAGE);
			ftfDesconto.requestFocus();
			return false;
		} */
		return true;
	}
