...
/**
 * Janela de consulta a dados de Doenças
 *
 * @author Inocencio
 */
public class BuscaDoenca extends javax.swing.JDialog {
	...
	public BuscaDoenca(JInternalFrame parent) {
		super();
		this.parent = parent;
		initComponents();
		carregarGrade();
	...
	private void carregarGrade() {
		try {
			...
			DoencaTableModel ptm = (DoencaTableModel) tbGrade.getModel();
			ptm.setDados(doencaDAO.listarTodos());
		} catch (Exception ex) {
	...
	private void tbGradeMouseClicked(java.awt.event.MouseEvent evt) throws Exception {// GEN-FIRST:event_tbGradeMouseClicked
		if (evt.getClickCount() == 2) {
			DoencaTableModel ptm = (DoencaTableModel) tbGrade.getModel();
			int linhaSelecionada = tbGrade.getRowSorter().convertRowIndexToModel(tbGrade.getSelectedRow());
			Doencas doenca = ptm.getRowValue(linhaSelecionada);

			if (parent instanceof CadastroCliente) {
