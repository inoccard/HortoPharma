...
public class CompraDAO implements IDAO<Compra> {
    ...
    public void inserir(Compra compra) throws Exception {
        ...
        String sql = "INSERT INTO TBCOMPRA (CODIGOFORNECEDOR, DATACOMPRAS, VALORTOTAL, SITUACAO) VALUES (?, ?, ?, ?)";
    ...
    public void alterar(Compra compra) throws Exception {
        Conexao c = new Conexao();
        String sql = "UPDATE TBCOMPRA SET CODIGOFORNECEDOR=?, DATACOMPRAS=?, VALORTOTAL=?, SITUACAO=? WHERE CODCOMPRAS=?";
    ...
    public ArrayList<Compra> listarTodos() throws Exception {
        ...
        String sql = "SELECT * FROM TBCOMPRA ORDER BY DATACOMPRAS DESC";
        PreparedStatement ps = c.getConexao().prepareStatement(sql);
        ...
        while (rs.next()) {
            ...
            compra.setCodigo(rs.getInt("CODCOMPRAS"));
            compra.setFornecedor(fornecedorDAO.recuperar(rs.getInt("CODIGOFORNECEDOR")));
            compra.setDataCompra(rs.getDate("DATACOMPRAS"));
            ...
            while (rsItem.next()) {
                ...
                ic.setCodigo(rsItem.getInt("CODIGO"));
                ic.setProduto(produtoDAO.recuperar(rsItem.getInt("CODIGOPRODUTO")));
                ic.setCompra(compra);
                ic.setQuantidade(rsItem.getInt("QUANTIDADE"));
                ic.setValorUnitario(rsItem.getDouble("VALORUNITARIO"));
                compra.addItem(ic);
            }
	...
	@Override
	public ArrayList<Compra> recuperarDoencasPorCliente(int codigo) throws Exception {
